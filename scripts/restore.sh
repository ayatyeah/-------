...
# ─── Фотографии ────────────────────────────────────────────────────────────
# Восстановленный store.json ссылается на снимки по именам файлов. Если сами
# файлы не вернуть, каталог поднимется с пустыми карточками — формально
# рабочий сайт без единой фотографии. Поэтому предлагаем и архив снимков.
UPLOADS="${UPLOAD_DIR:-/data/uploads}"
LAST_UP=$(find "$BACKUP_DIR" -name 'uploads-*.tar.gz' -type f 2>/dev/null | sort | tail -1)

if [ -n "$LAST_UP" ]; then
  log "нашёлся архив фотографий: $(basename "$LAST_UP")"
  mkdir -p "$UPLOADS"
  # Распаковываем поверх: файлы с одинаковыми именами и так одинаковые
  # (имя содержит случайный хвост и не переиспользуется), а лишние снимки
  # на диске безобиднее недостающих.
  tar -xzf "$LAST_UP" -C "$UPLOADS"
  log "фотографии восстановлены в $UPLOADS"
else
  log "архивов фотографий не найдено — если снимки были, восстановите их вручную"
fi
