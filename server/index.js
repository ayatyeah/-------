 ...
 *
 * Данные лежат в data/store.json и, если она настроена, дублируются в
 * PostgreSQL (см. server/store.js и server/db.js). Сервер ходит только
 * через методы store.* и про то, где именно лежат данные, не знает.
 *
 * Важно для понимания живучести: доступность базы на работу эндпоинтов не
 * влияет никак. Упавшая база — это строчка в логе и жёлтая отметка в
 * админке, а не отказ сайта.
 */
...
const CSP = [
  ...
  /* Карта на «Контактах» встраивается через iframe 2ГИС или Яндекс.Карт,
     поэтому их домены здесь перечислены явно. Обрати внимание: карта
     грузится ТОЛЬКО после нажатия посетителем (см. src/pages/Contacts.jsx).
     До нажатия ни один запрос на эти домены не уходит, и IP посетителя
     третьей стороне не достаётся — иначе встроенная карта тихо обходила бы
     тот самый выбор «Только необходимые», который сайт честно предлагает. */
  "frame-src 'self' https://2gis.kz https://*.2gis.kz https://*.2gis.com https://yandex.ru https://*.yandex.ru https://yandex.kz https://*.yandex.kz",
  "worker-src 'self'",
  "media-src 'self'",
  "manifest-src 'self'",
].join('; ')
...
/**
 * Что с базой данных. Только для админки — здесь видно и текст ошибки, и
 * счётчики строк, а такое наружу не отдают.
 *
 * Зачем отдельный экран: без него недоступность базы замечают через месяц,
 * когда понадобится выгрузка. Сайт-то работает.
 */
app.get('/api/admin/db', requireAdmin, wrap(async (_req, res) => {
  res.json(await db.statusDetailed())
}))

/**
 * Кнопка «перелить заново»: содержимое памяти целиком отправляется в базу.
 *
 * Нужна редко, но нужна: после долгого простоя базы, после ручного
 * вмешательства в таблицы или просто чтобы убедиться, что копия сходится.
 * Операция безопасна в любой момент — она приводит базу к тому, что сейчас
 * показывает сайт, и ничего не удаляет из памяти и из файла.
 */
app.post('/api/admin/db/resync', requireAdmin, limitAdminWrite, wrap(async (_req, res) => {
  const r = await db.resync()
...
app.get('/api/health', wrap((_req, res) => {
  ...
  const d = db.status()
  /* Состояние базы сюда попадает СПРАВОЧНО и на код ответа не влияет.
     Это принципиально: по этому адресу Docker решает, живой ли контейнер, и
     привязать его к базе значило бы получить перезапуск сайта каждый раз,
     когда база перезагружается. Сайт без базы полностью работоспособен —
     значит, он здоров.
     Наружу отдаём одно слово состояния, без адресов и текстов ошибок:
     адрес открытый, и помогать разведке он не должен. */
  res.status(ok ? 200 : 503).json({
    ok,
    uptime: Math.round(process.uptime()),
    db: d.configured ? d.state : 'off',
  })
}))
...
const server = app.listen(PORT, HOST, () => {
  ...
  /* Слой базы поднимается ПОСЛЕ listen и намеренно не ожидается: сайт уже
     принимает запросы, а подключение, проверка схемы и первичный перенос
     данных идут фоном. Если база не отвечает — здесь ничего не сломается,
     модуль сам уйдёт в режим переподключения.

     seeded говорит слою базы, что память сейчас содержит начальные данные,
     а не настоящие: значит, файл потеряли, и поднимать сайт надо из базы,
     если в ней что-то есть. */
  db.start({ seeded }).catch((e) => {
    console.error('Слой базы данных не поднялся (сайт работает на файле):', e.message)
  })
})

/* Сообщения о состоянии базы — в тот же Telegram, куда приходят заявки.
   Владелец узнаёт о проблеме сразу, а не когда понадобится выгрузка.
   Канал необязательный: не настроен — просто не отправится. */
db.attachAlert((msg) => {
...
function shutdown(signal, code = 0) {
  ...
  // Файл — первым и синхронно. Это гарантированный путь: он не зависит ни
  // от сети, ни от того, жив ли контейнер с базой.
  store.flush()

  /* База — следом и по возможности. Ждать её дольше пары секунд нельзя:
     stop_grace_period в compose равен 20 с, и если упереться в мёртвое
     соединение, Docker прибьёт процесс по таймауту. Данные при этом уже
     на диске, а база догонит при следующем запуске полной сверкой —
     терять здесь нечего. */
  const закрытьБазу = Promise.race([
    db.flushNow().then(() => db.close()),
